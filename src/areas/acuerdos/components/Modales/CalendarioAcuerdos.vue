...
<script setup>
  ...
  {
    const fecha                     = getDateToStr( selectedDate.value, "UTC" )
  ...
  function crearEventosLocales()
  {
    ...
    {
      const titulo    = `${ant.tipoSelect.label} ${formatoPrecio( ant.valor, "decimales-no" )}`
      const detalles  = `${ant.estadoSelect.label} ${formatoPrecio( ant.valor, "decimales-no" )}`
  ...
  async function crearEventosNube()
  {
    ...
    {
      let titulo        = mayusculasPrimeraLetra( evento.titulo.split(" ").slice(2).join(" ") )      
      ...
      const detalles    = `${evento.creadorLabel} ${titulo} - ${horaAmPm( evento.creacion )}`
  ...
  {      
    ...
    evento.type       = 0
    evento.allDay     = false
    evento.date       = typeof fecha === "string" ? fecha : fechaCorta( fecha )
    if(conHora && typeof fecha === "object")
      evento.hora     = horaAmPm( fecha )
