...
export function useReciboCajaPDF()
{
  ...
  let aviso   : Function
  let pedido   : IAcuerdo    = new Acuerdo( TIPO_ACUERDO.PEDIDO )
  ...
  async function generarPDF( acuerdo : IAcuerdo ) : Promise<string>
  {
          limpiar()
          mostrarAviso()
    pedido                   = acuerdo
  ...
  {
    algunGrupoConSubTotal   = pedido.proGrupos.some( g => g.conTotal )   
    doc.area                = !!pedido.tercero.area ? pedido.tercero.area : pedido.comercial.area
  ...
  function generarCabezoteLogo()
  {
    ...
    doc.y               = 17
    doc.setFont         ( 16, 60 )
    const titulo        = pedido.esEstadoNoValidado ? "NO VALIDO" : "COTIZACIÓN"
  ...
  {
    if(!pedido.titulo) return

    doc.x               = doc.anchoMitad
    doc.y               += 13
    doc.setFont         ( 16, 50 )
    pdf.text            ( pedido.titulo, doc.x, doc.y, { align: "center", renderingMode: 'fillThenStroke' })
  }
  ...
  async function generarProductos()
  {
    ...
    for (const grupo of pedido.proGrupos)
    {
      const altoBarra           = pedido.proGrupos.length > 1 ? 14 : 10
  ...
  {
    if (!algunGrupoConSubTotal || pedido.proGrupos.length < 2) return
    
    const subtotales            = pedido.proGrupos.filter(g => g.conTotal)
    const altoBarra             = 16
  ...
  function generarTotales()
  {
    ...
    const nombreCondiciones     = "Condiciones de pago:\n"
                                  + ( !!pedido.formaPago.id        ? "Forma de pago:\n"      : "")
                                  + ( !!pedido.metodoEntrega.id    ? "Método de entrega:\n"  : "")                                  
                                  + "Precios validos hasta:\n" 
                                  + ( !!pedido.tiempoEntrega.id    ? "Tiempo de entrega:"    : "")    
    pdf.text                    ( nombreCondiciones, doc.margenIzq - 4, posY, { align: "right", lineHeightFactor: 1.4, renderingMode: 'fillThenStroke' })

    const condiciones             = pedido.condicionPago.descripcion + "\n"
                                    + ( !!pedido.formaPago.id      ? `${pedido.formaPago.label}\n`      : "")
                                    + ( !!pedido.metodoEntrega.id  ? `${pedido.metodoEntrega.label}\n`  : "")
                                    + `${pedido.fechaFinValidezCorta}. Después de esta fecha deberán re-confirmarse.\n`
                                    + ( !!pedido.tiempoEntrega.id
                                        ? `${pedido.tiempoEntrega.label} para producción y entrega, proyectados después de recibido y legalizado el anticipo. Condicionado con los horarios a programar con el supervisor designado por el cliente.`
                                        : ''
                                      )
    ...
    // * //////////////////////////////////////////////// Valores totales
    let valores                 =  formatoPrecio(pedido.totalSinDescu,   "decimales-si") + "\n"
    valores                     += formatoPrecio(pedido.descuentoValor,  "decimales-si") + "\n"
    valores                     += formatoPrecio(pedido.totalConDescu,   "decimales-si") + "\n"
    valores                     += formatoPrecio(pedido.ivaValor,        "decimales-si") + "\n"
    valores                     += formatoPrecio(pedido.totalConIva,     "decimales-si")
  ...
  {
    if(!pedido.notaPublica) return

    const copia = doc.y
    const notaSplit             = pdf.splitTextToSize(pedido.notaPublica, doc.ancho - 30  ) as Array<string>    
...
/*     html2canvas(document.querySelector("#capture")).then(canvas => {
  ...
  {
    pedido.proGrupos.forEach(grupo => {
  ...
  function generarCabezoteDatos()
  {
    ...
    textoDer            =  pedido.comercial.nombreCompleto + "\n"
    textoDer            += pedido.comercial.cel + "\n"
    textoDer            += pedido.comercial.correo + "\n"
    textoDer            += pedido.condicionPago.label + "\n"
    textoDer            += pedido.fechaFinValidezCorta + "\n"
    textoDer            += pedido.ref
    doc.x               += 7
    doc.setColor        ( 40 )
    ...
    textoDer            =  pedido.pdfNombre + "\n"
    textoDer            += pedido.pdfContacto + "\n"
    textoDer            += pedido.pdfCorreo + "\n"
    textoDer            += pedido.pdfCiudad + "\n"
    textoDer            += fechaLarga( new Date() ) + "\n"
    textoDer            += pedido.conIVA ? textoConIVA : textoSinIVA
    doc.x               = 63
    doc.setColor        ( 40 )
    pdf.text            (textoDer, doc.x, doc.y, { align: "left", lineHeightFactor: 1.5 })
