<template>
  <ventana                      minimizar
    :titulo                     ="cotizacion.estado === ESTADO_CTZ.NO_GUARDADO ? 'Consultar productos - No se guardan en cotizacion... por ahora. enserio🙏😱😩🤯' : 'Productos'"
    icono                       ="mdi-package-variant-closed"
    padding-contenido           ="0"
    >
    <template                   #barra
      v-if                      ="cotizacion.estado <= 0" 
      >
      <!-- //* ///////////////////////////////////////////////////////////// Boton nuevo grupo   -->
      <q-btn
        v-bind                  ="btnBaseSm"
    ...
    <q-list                   
      ...
      <q-expansion-item         expand-separator default-opened dense expand-icon-toggle dense-toggle
        v-for                   ="(grupo, index) of cotizacion.proGrupos"
        :index                  ="grupo.index"
        ...
        <template               #header>
          ...
          <q-item-section class ="q-my-sm">
            <editar-grupo
              ...
              @borrar-todo      ="( ( grupo ) => borrarGrupo(grupo, 'borrar'    ) )"
              @borrar-conservar ="( ( grupo ) => borrarGrupo(grupo, 'conservar' ) )"
            />
          </q-item-section>
          <q-item-section       side
            v-if                ="cotizacion.estado <= 0" 
    ...
    <q-dialog                   maximized
      ...
      <buscar-productos
        ...
        :con-iva                ="cotizacion.tercero.aplicaIVA"
...
<script setup lang="ts">
  ...
  import    editarGrupo         from "src/areas/acuerdos/.components/EditarGrupo.vue"
  ...
  const totalTem                = computed( ()=> 
    !!cotizacion.value.proGrupos.length ? cotizacion.value.proGrupos[0].totalConDescu : 0
  )
  const largo                   = computed( ()=> cotizacion.value.proGrupos.length )
  ...
  onMounted(()=>{
   // if(cotizacion.value.estado  === ESTADO_CTZ.NO_GUARDADO)
   //   crearNuevoGrupo()
  })

  watch( () => cotizacion.value.productos, (newProductos, oldProductos) => {
    if(cotizacion.value.estado  !== ESTADO_CTZ.NO_GUARDADO && !newProductos.length) // Crea un grupo si no hay productos
      crearNuevoGrupo()
  ...
  {
    if(lineas.length > 0)
      cotizacion.value.proGrupos  = GrupoLineas.productosAgrupoDeProductos( lineas )
    else 
      crearNuevoGrupo()
  }
  ...
  async function eliminarLineaDesdeBusqueda( idProducto : number )
  {
    ...
    let indexGrupo = cotizacion.value.proGrupos.findIndex( g => g.index === grupoSelect.value.index ) 
    cotizacion.value.proGrupos[indexGrupo] = grupoSelect.value
  }
  ...
  {
    cotizacion.value.productos      = GrupoLineas.deGruposAProductos( cotizacion.value.proGrupos )
    ...
    if(!!cotizacion.value.id)
    {
      for (const linea of cotizacion.value.productos)
      {
        ...
        )
        {
          ...
          let { data, ok }          = await apiDolibarr("crear", "lineaCotizacion", lineaApi, cotizacion.value.id)
          ...
          if(ok)
          {
            ...
            const grupo             = cotizacion.value.proGrupos.at(-1)
        ...
        if(linea.borrar)
        {
          let {ok, data}        = await apiDolibarr("borrar", "lineaCotizacion", linea.lineaId, cotizacion.value.id)
    ...
    borrarGruposSinProductos()
    actualizarArrayGrupos( cotizacion.value.productos )
  ...
  {
    let ids = cotizacion.value.productos.map( p => p.lineaId ).join()
    let ok  = await ordenarLineas(ids, cotizacion.value.id )
    if(!ok && cotizacion.value.productos.length >= 1)
      aviso("negative", "Error al ordenar")
  }

  function borrarGruposSinProductos()
  {
    if(cotizacion.value.proGrupos.length === 1 ) return 

    for (const grupo of cotizacion.value.proGrupos)
    {
  ...
  {
    let indexGrupo = cotizacion.value.proGrupos.findIndex( g => g.index === grupo.index ) 
    cotizacion.value.proGrupos[indexGrupo] = grupo
    cambioEnGrupos('buscarProductos') 
  }

  async function crearNuevoGrupo()
  {
    let index                   = cotizacion.value.proGrupos.length
    let nuevoGrupo              = new GrupoLineas( index )
    cotizacion.value.proGrupos.push( nuevoGrupo )
    ...
    // let { data:dTit, ok:okTitu }= await apiDolibarr("crear", "lineaCotizacion", lineaTituloApi,  cotizacion.value.id)
    // let { data:dSub, ok:okSub  }= await apiDolibarr("crear", "lineaCotizacion", lineaSubtotalApi, cotizacion.value.id)
  ...
  function borrarGrupo( grupo : IGrupoLineas, accion : "conservar" | "borrar" )
  {
    if(accion                   == 'conservar')
    {
      ...
      cotizacion.value.proGrupos[ indexGrupoMover ].productos.push( ...grupo.productos )      
    }
      
    cotizacion.value.proGrupos.splice(grupo.index, 1)            // Borrar Grupo
    cotizacion.value.proGrupos.forEach( (g, i)  => g.index = i ) // Reasignar Index
